...
# Loop through each cryptocurrency symbol to fetch historical data
for symbol in crypto_symbols:
    ...
    # Create a DataFrame and set the date index with explicit frequency
...
# Create a figure and axis for the combined forecast plot
plt.figure(figsize=(12, 6))

# Plot the forecasts for each symbol together
for symbol in crypto_symbols:
    crypto_series = crypto_data[symbol]['close']
    arima_model = ARIMA(crypto_series, order=order)
    arima_results = arima_model.fit()
...
plt.xlabel('Date')
plt.ylabel('Price')
plt.title('Combined ARIMA Cryptocurrency Price Forecasts')
plt.legend()
plt.grid(True)
plt.show()
